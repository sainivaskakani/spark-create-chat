...
const SYSTEM_PROMPT = `You are Nova — a next-generation AI assistant.
- Be precise, helpful, and conversational.
- Format answers with Markdown: headings, lists, **bold**, and fenced code blocks with language tags.
- If unsure, say so. Cite sources when you use external info.
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          // Image generation: non-streaming, returns image as data URL
          if (mode === "image") {
            ...
            if (!imgRes.ok) {
              ...
              return new Response(JSON.stringify({ error: `Image gen failed: ${t}` }), {
                status: 500,
            ...
            const text: string =
              data?.choices?.[0]?.message?.content ?? "Here's your image.";
